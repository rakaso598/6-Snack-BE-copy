...
export class AuthRepository {
   ...
   */
  static async findUserByEmailWithCompany(email: string) {
   ...
   */
  static async findCompanyByBizNumber(bizNumber: string) {
   ...
   * @returns 생성된 회사 정보
   */
  static async createCompany(data: { name: string; bizNumber: string }, prismaTransaction: Prisma.TransactionClient) {
   ...
   */
  static async createUser(data: { email: string; name: string; password: string; role: Role; companyId: number }, prismaTransaction: Prisma.TransactionClient) {
   ...
   */
  static async updateUserRefreshToken(userId: string, hashedRefreshToken: string | null) {
   ...
   * @param prismaTransaction - 트랜잭션 내에서 사용할 Prisma 트랜잭션 클라이언트
   */
  static async updateInviteToUsed(inviteId: string, prismaTransaction: Prisma.TransactionClient) {
   ...
   * @returns 콜백 함수의 결과
   */
  static async runInTransaction<T>(callback: (prismaTransaction: Prisma.TransactionClient) => Promise<T>): Promise<T> {
