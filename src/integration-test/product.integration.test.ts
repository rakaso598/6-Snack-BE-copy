...
describe("Product Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    parentCategory = await prisma.category.create({ data: { name: "음료" } });
    childCategory = await prisma.category.create({ data: { name: "콜라", parentId: parentCategory.id } });
    anotherCategory = await prisma.category.create({ data: { name: "과자" } });
