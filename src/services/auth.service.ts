...
export class AuthService {
   ...
   * @throws {AppError} - 이메일/사업자 등록 번호 중복 시
   */
  static async signUpSuperAdmin(data: { email: string; name: string; password: string; companyName: string; bizNumber: string }) {
    ...
    if (existingCompanyByBizNumber) {
      throw new AppError('이미 등록된 사업자 등록 번호입니다.', 409);
    }
   ...
   * @returns 생성된 사용자 정보
   * @throws {AppError} - 유효하지 않거나 만료된 초대, 이미 사용된 초대, 이메일 중복 시
   */
  static async signUpViaInvite(inviteId: string, password: string) {
    const invite = await AuthRepository.findInviteById(inviteId);

    if (!invite) {
      throw new AppError('유효하지 않은 초대 링크입니다.', 404);
    }
    if (invite.isUsed) {
      throw new AppError('이미 사용된 초대 링크입니다.', 409);
    }
    if (invite.expiresAt && new Date() > invite.expiresAt) {
      throw new AppError('만료된 초대 링크입니다.', 410);
    }
    ...
    if (existingUser) {
      ...
      throw new AppError('이미 가입된 이메일입니다. 초대 링크가 사용 완료 처리되었습니다.', 409); // 또는 200 응답으로 변경 가능
    }
  ...
  static async login(email: string, password_plain: string) {
    ...
    if (!user) {
      throw new AppError('잘못된 이메일 또는 비밀번호입니다.', 400);
    }
  ...
  static async refreshAccessToken(currentRefreshToken: string) {
    ...
    try {
      ...
      if (error instanceof jwt.TokenExpiredError) {
        ...
        throw new AppError('유효하지 않은 리프레시 토큰입니다. 다시 로그인해주세요.', 403);
      } else {
        console.error('[리프레시 토큰 검증 오류]', error);
        throw new AppError('리프레시 토큰 검증 중 알 수 없는 오류가 발생했습니다.', 500);
      }
    }
    ...
    if (!user || !user.hashedRefreshToken) {
      throw new AppError('사용자 또는 유효한 리프레시 토큰을 찾을 수 없습니다. 다시 로그인해주세요.', 401);
    }
    ...
    if (!isRefreshTokenValid) {
      ...
      throw new AppError('유효하지 않거나 이미 사용된 리프레시 토큰입니다. 다시 로그인해주세요.', 403);
    }
   ...
   * @throws {AppError} - 로그아웃 처리 중 오류 발생 시
   */
